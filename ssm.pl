...
sub generateCpp11 {
	...
	print OFILE "	${Machine}(std::tuple<Args...> t) {$/";
	print OFILE "		initNeededMethods();$/";
	print OFILE "		auto t2 = std::tuple_cat(std::make_tuple(&timer), t);$/";
	print OFILE "		autoRegisterAll(&actionMap, &conditionMap, t2, neededMethods);$/";
